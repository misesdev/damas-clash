...
/**
 * Immutable game engine — each mutating operation returns a new instance.
 * React state: `const [engine, setEngine] = useState(() => GameEngine.initial())`
 */
export class GameEngine {
  ...
  /** Returns updated engine after selecting/deselecting a piece at (row, col). */
  ...
  /**
   * Applies the move targeting (row, col).
   * Returns the moving piece, the move, and the resulting engine — or null if invalid.
   */
  applyMoveAt(
    row: number,
    col: number,
