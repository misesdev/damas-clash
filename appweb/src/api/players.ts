...
export const getPlayer = (token: string, playerId: string): Promise<PlayerProfile> =>
  request(`/api/players/${playerId}`, { headers: auth(token) });

export const updateLightningAddress = (
  token: string,
  playerId: string,
  address: string | null,
): Promise<PlayerProfile> =>
  request(`/api/players/${playerId}/lightning-address`, {
    method: 'PUT',
    headers: auth(token),
